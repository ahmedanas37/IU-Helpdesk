...
if(isset($_POST["permission_id"]) && !empty($_POST["permission_id"])){
    ...
    $stmt = $pdo->prepare("UPDATE role_permission SET role_id=?,permission_id=? WHERE permission_id=?");

    if(!$stmt->execute([ $role_id,$permission_id,$permission_id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: role_permission-read.php?permission_id=$permission_id");
    }
} else {
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
					...
					$permission_id = htmlspecialchars($row["permission_id"]);
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
						...
						<div class="form-group">
                                ...
                                <span class="form-text"><?php echo $permission_id_err; ?></span>
                            </div>
