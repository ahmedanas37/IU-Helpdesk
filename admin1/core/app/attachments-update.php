...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $vars = parse_columns('attachments', $_POST);
    $stmt = $pdo->prepare("UPDATE attachments SET comment_id=?,file_name=?,file_path=?,created_at=? WHERE id=?");

    if(!$stmt->execute([ $comment_id,$file_name,$file_path,$created_at,$id  ])) {
        echo "Something went wrong. Please try again later.";
        header("location: error.php");
    } else {
        $stmt = null;
        header("location: attachments-read.php?id=$id");
    }
} else {
            ...
            <div class="row">
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <a href="attachments-index.php" class="btn btn-secondary">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
