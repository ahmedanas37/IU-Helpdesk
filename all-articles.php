...
if (isset($_GET['category'])) {
    ...
    // Now, fetch articles for the selected category from the database using SQL
    $articleSql = "SELECT * FROM articles WHERE category = '$selectedCategory'";
    $articleResult = mysqli_query($conn, $articleSql); // Assuming $connection is your database connection

    // Create the dynamic HTML content for the articles
    $articlesHtml = '';
    while ($articleRow = mysqli_fetch_assoc($articleResult)) {
        $articlesHtml .= '
            <div class="dx-article dx-article-block">
                <h4 class="h6 dx-article-title"><a href="article-details.php?id=' . $articleRow['id'] . '">' . $articleRow['title'] . '</a></h4>
                <div class="dx-article-text">
                    <p class="mb-0">' . $articleRow['content'] . '</p>
                </div>
                <a href="article-details.php?id=' . $articleRow['id'] . '" class="dx-btn dx-btn-link d-flex dx-article-link">Read More <span class="icon pe-7s-angle-right"></span></a>
            </div>';
    }
    ...
    <div class="dx-box dx-box-decorated">
        <div class="dx-blog-post">
            <div class="dx-blog-post-box pt-30 pb-30">
                <h2 class="h4 mnt-5 mb-8"><?php echo $selectedCategory; ?></h2>
                <!-- START: Breadcrumbs -->
